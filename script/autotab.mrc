; -----------------------------
; Auto Tab (original code par iinc.mrc@masquerader.com)
; Corrige et Modifié par Klarth x_x
; -----------------------------

#features on
ON *:START:{
  if ($hget(groups,tabcomp) == $null) { hadd -m groups tabcomp 1 }
  if ($hget(groups,tabcomp) == 1) { 
    splash_text 3 Chargement du script $nopath($script) ...
    .enable #features.tabcomp 
    if ($timer(edbox) == $null) { edbox_chk }
  }
  else { .disable #features.tabcomp }
}
...
on *:unload:{ 
  ...
  if ($hget(groups,tabcomp)) { hadd groups tabcomp 0 }
}
#features end
...
alias -l completedNick {
  if ($1 == $null) { return $iif($ri(tabcomp,use) == $null,1,$ri(tabcomp,use)) }
  else { return $replace($iif($ri(tabcomp,format) == $null,<nick>:,$ri(tabcomp,format)),<nick>,$1) }
}
...
alias -l edit_change {
  ...
  if ($ri(tabcomp,use) == 1) && ($ri(tabcomp,bsundo) == 1) && ($mouse.key !& 4) {
  ...
  if ($ri(tabcomp, use) == 0) || ($right($editbox($active),1) != $trig) || ($mid($editbox($active),-2,1) != $trig) {
    edit_clear
    return
  }
